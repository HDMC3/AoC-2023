...
const camelHands = content.split("\n")
    .map((line) => {
        ...
        let handType: number | undefined;
        if (hand.includes("J")) {
            const possibleTypes = hand.split("")
                .filter((c) => c != "J")
                .map((c) => getHandType(hand.replace(/J/g, c)));

            handType = possibleTypes.length == 0
                ? HandType.FiveOfKind
                : Math.max(...possibleTypes);
        }

        const camelHand: CamelHand = {
            hand,
            bid: Number(bid),
            type: handType ?? getHandType(hand),
        };
        return camelHand;
    });
